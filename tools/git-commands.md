...
git stash push -m “description of change”   # just like manual commit
git stash list                       # list all stashes
git stash pop                        # delete the recent stash and applies it
git stash pop 1 # delete the stash and add the changes of that stash
...
# Git Merge
git checkout <branch_to_merge> //where to merge
git pull
git checkout <branch_from> // branch which has merge conflicts
git merge <branch_to_merge>
git status // resolve all the conflicts in red color
git add --all
git commit -m "resolved merge conflicts"
git push

# Cherry Pick
lets you copy a specific commit and superglue it on the end of your current branch.

git checkout <your_current_branch>
git cherry-pick <commit_hash_of_other_branch_commit>

git rebase -I master // pull all the changes happened on master branch while you were working on your branch
git fetch origin and git rebase -p origin/develop // when you are about to commit but want to get any other changes happened on develop branch
